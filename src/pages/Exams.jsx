...
function Exams({ exams, setExams, darkMode }) {
  ...
  const addExam = () => {
    if (!examTitle || !course || !examDate) return;
    setExams([...exams, { id: Date.now(), examTitle, course, examDate, notes }]);
    setExamTitle(""); setCourse(""); setExamDate(""); setNotes("");
  };
  ...
  const input = dm
    ? "bg-white/5 border border-white/10 text-white placeholder-slate-500 rounded-2xl px-5 py-4 outline-none focus:ring-4 focus:ring-orange-500/20 focus:border-orange-400 transition-all"
    : "bg-white border border-slate-200 rounded-2xl px-5 py-4 outline-none focus:ring-4 focus:ring-orange-100 transition-all";
    ...
    <div className="space-y-8">
      ...
      <div className={`${cardForm} rounded-[32px] p-8 shadow-xl`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
          <input type="text" placeholder="Exam title" value={examTitle} onChange={(e) => setExamTitle(e.target.value)} className={input} />
          ...
          <input type="text" placeholder="Notes" value={notes} onChange={(e) => setNotes(e.target.value)} className={input} />
        </div>

        <button onClick={addExam} className="mt-6 bg-gradient-to-r from-orange-500 to-red-500 text-white px-7 py-4 rounded-2xl font-bold shadow-xl hover:scale-[1.02] transition-all duration-300">
          Add Exam
        </button>
      </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-between mt-2">
                  <div className={`w-16 h-16 rounded-3xl flex items-center justify-center ${dm ? "bg-orange-500/20" : "bg-orange-100"}`}>
                    <FiCalendar className={dm ? "text-orange-400" : "text-orange-500"} size={30} />
                  </div>
                  <button onClick={() => deleteExam(exam.id)}
                    className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-300 ${dm ? "bg-white/10 text-slate-400 hover:bg-red-500 hover:text-white" : "bg-slate-100 text-slate-500 hover:bg-red-500 hover:text-white"}`}
                  ><FiTrash2 size={20} /></button>
                </div>

                {/* CONTENT */}
                <div className="mt-8">
                  <h2 className={`text-3xl font-bold ${textMain}`}>{exam.examTitle}</h2>
                  <p className={`mt-2 ${textSub}`}>{exam.course}</p>

                  <div className="mt-6 flex items-center gap-3 text-orange-400 font-semibold">
                    <FiClock />
                    <span>{exam.examDate}</span>
                  </div>

                  <div className={`mt-6 inline-flex px-4 py-2 rounded-full font-bold text-sm ${
                    daysLeft <= 7
                  ...
                  {exam.notes && (
                    <div className={`mt-6 rounded-2xl p-4 ${dm ? "bg-white/5" : "bg-slate-50"}`}>
                      <p className={textSub}>{exam.notes}</p>
                    </div>
                  )}
                </div>
              </div>
